...
- Enroll another GameNode installation as a Remote Node: durable node identity, secure pairing-token enrollment, an authenticated machine-to-machine Node API, and health/capability status - without giving up local autonomy. General remote server management (start/stop/restart/edit/delete an existing remote server) is not yet implemented; a deterministic cluster placement decision, and typed native/container server creation on the selected node (local or remote), are - see [Remote Nodes](#remote-nodes-v05a) and [Cluster Scheduling](#cluster-scheduling-v06) below.
...
v0.5A is read-only foundation: node identity, pairing/enrollment, the Node API, the remote-node registry, health/capability status, and the Nodes UI. General remote server create/edit/start/stop/restart/kill and remote console/files remain a later milestone (v0.5B, not started). v0.6 adds one narrow, typed exception - remote provisioning (server creation via an Egg/template) - described next. See [`docs/adr/0007-remote-node-foundation.md`](docs/adr/0007-remote-node-foundation.md) for the full trust model.
