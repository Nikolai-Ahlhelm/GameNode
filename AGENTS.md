...
- Do not silently implement the next milestone. Automatic game updates, backups, firewall/NAT automation, marketplace/community catalogs, and credentialed Steam login remain out of scope. Multi-node management is now partially in scope (v0.5A/v0.5B/v0.5C: enrollment, remote server lifecycle, remote console/files/monitoring - all forwarded to each node's own local authority, never a shared cluster database or second lifecycle implementation); automatic server migration, failover, controller election, and any generic multi-node orchestrator remain out of scope.
...
- `internal/placement`: the v0.6 Cluster Scheduling foundation. A pure, deterministic placement DECISION engine (`Decide`) over an already-built candidate list (this node plus every enrolled Remote Node), and read-only candidate builders (`LocalCandidate` from `servers.Service`/`nodeidentity`, `RemoteCandidates` from the v0.5A Remote Node registry). It never mutates a server anywhere, never contacts a remote node itself, and is completely independent of `internal/scheduler` (see below - do not confuse the two). See `docs/adr/0009-cluster-scheduling-decision-vs-execution.md`.
...
- v0.6 (`internal/placement`) adds a deterministic, tenant-isolated, RBAC/audit-gated Cluster Scheduling decision engine across this node and every enrolled Remote Node, plus `GET /api/v1/cluster/capacity`, `POST /api/v1/cluster/placement`, and explicit `POST /api/v1/cluster/placement/execute` APIs. The pure placement package never mutates; execution delegates to the selected node's normal create authority. See `docs/adr/0009-cluster-scheduling-decision-vs-execution.md`.
- v0.4 Container-backed Egg Runtime and v0.5A Remote Node Foundation were developed concurrently on separate branches/worktrees and have since been integrated onto a common base. v0.5A (`internal/nodeidentity`, `internal/nodes`, `internal/remote`, the `/api/v1/node/*` and `/api/v1/remote-nodes*` API, migration `026_remote_nodes.sql`, and the read-only Nodes UI) does not depend on v0.4 and did not originally advertise an Egg Runtime capability. Now that v0.4 is integrated, `internal/nodeidentity`'s capability list includes a truthful `egg_container_runtime` entry (see section 5 above); `internal/nodes`/`internal/remote` still do not import or understand Egg internals.
- v0.5B (Remote Server Management) and v0.5C (Remote Operational Hardening) add remote server Create/Edit/Delete/Start/Stop/Restart/Kill, a bounded WebSocket console relay with polling fallback, sandboxed text/binary files, and remote monitoring, through the `remote_server_management`/`remote_console`/`remote_files`/`remote_monitoring` capabilities and `/api/v1/node/servers[...]`/`/api/v1/remote-nodes/{id}/servers[...]` surfaces. Every operation forwards to the target node's own `internal/servers.Service`/`internal/filesystem.Service`/`internal/console.Manager` - never a second lifecycle implementation. Backend, tests, and controller-facing Nodes UI are implemented (see ADRs 0010/0011).
