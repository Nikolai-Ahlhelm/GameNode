...
- `migrations`: ordered embedded SQL migrations. The current highest file is `023_remote_nodes.sql`.
...
- Current groups are Server (`View/Create/Edit/Delete/Start/Stop/Restart/Kill`), Console (`View/Send`), Files (`View/Edit/Upload/Download/Delete/Rename`), Ports (`View/Manage`), identity (`Users`, `Groups`, `Roles`: `View/Manage`), platform (`Settings.View/Manage`), Templates (`View/Manage`), `Monitoring.View`, `Audit.View`, Tenants (`View/Manage`, administering tenant entities themselves - never resources inside one), and Node (`View/Manage`, administering this installation's Remote Node registry/pairing - never a remote node's own resources, which stay behind that node's own RBAC). The code catalog is definitive.
...
- v0.4 Container-backed Egg Runtime and v0.5A Remote Node Foundation were developed concurrently on separate branches/worktrees. v0.5A (`internal/nodeidentity`, `internal/nodes`, `internal/remote`, the `/api/v1/node/*` and `/api/v1/remote-nodes*` API, migration `023_remote_nodes.sql`, and the read-only Nodes UI) does not depend on v0.4 and does not advertise an Egg Runtime capability until a branch that actually implements it does. v0.5A is identity, pairing/trust, a read-only Node API, and registry/health foundation only - remote Server Create/Edit/Start/Stop/Restart/Kill/Console/Files/provisioning remain unimplemented until v0.5B.
