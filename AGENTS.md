...
- v0.6 (`internal/placement`) adds a deterministic, tenant-isolated, RBAC/audit-gated Cluster Scheduling placement DECISION engine across this node and every enrolled Remote Node, plus `GET /api/v1/cluster/capacity`, `POST /api/v1/cluster/placement`, and explicit `POST /api/v1/cluster/placement/execute` APIs. Execution delegates local and remote container creation to the existing typed `provisioning.Service` path; `internal/placement` never mutates a server or imports Docker/runtime code. v0.5B/v0.5C separately provide remote server lifecycle, console, files, monitoring, controller proxies, RBAC/audit, and UI through the target node's own services. See ADRs 0009, 0010, and 0011.
- v0.4 Container-backed Egg Runtime and v0.5A Remote Node Foundation were developed concurrently on separate branches/worktrees and have since been integrated onto a common base. v0.5A provides identity, pairing/trust, and the registry foundation; v0.5B/v0.5C add typed remote Server Create/Edit/Delete/Start/Stop/Restart/Kill, bounded console relay/polling, sandboxed text/binary files, monitoring, controller proxies, RBAC/audit, and Nodes UI. `internal/nodes`/`internal/remote` still do not import or understand Egg internals. The `egg_container_runtime` capability remains a truthful additive capability from the v0.4 integration.
