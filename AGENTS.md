...
- v0.6 (`internal/placement`) adds a deterministic, tenant-isolated, RBAC/audit-gated Cluster Scheduling placement DECISION engine across this node and every enrolled Remote Node, plus its `GET /api/v1/cluster/capacity`/`POST /api/v1/cluster/placement` API. It deliberately never mutates any server, local or remote: see `docs/adr/0009-cluster-scheduling-decision-vs-execution.md` for the Decision vs. Execution boundary and why remote placement execution needs the still-unimplemented v0.5B first.
