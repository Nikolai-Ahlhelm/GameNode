...
Schema v2 adds the `managed-launch` format for games that read their settings
from process arguments or the environment rather than a file. It declares no
`target`, `section`, `container_property`, `initialization`, or
`post_start_only`. Each field replaces `property` with a `binding` chosen from
the closed whitelist `launch-value`, `launch-flag`, `launch-secret`,
`environment-value`, and `environment-secret`. The `argument` and `name` values
are reviewed adapter data; contributors never let a user supply an argument
name. `launch-flag` and the optional `true_value`/`false_value` mapping are
boolean-only, and the two secret bindings require a `secret`, `sensitive` field.

A managed field key must still match a declared template variable of the same
type, so provisioning has an initial value, and that key must **not** appear as
a placeholder in any base launch. Static arguments stay in `platform_launches`;
per-server settings belong to the adapter. Bump both the template and adapter
versions and update `catalog.json` in the same change. Only add a game option
you have verified against upstream documentation or a real installation.
