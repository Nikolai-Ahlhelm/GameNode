...
1. Create one game directory below the appropriate category and add its schema-versioned `template.json`.
2. Add or update its entry in `catalog.json`; keep IDs stable and increment the
   template version whenever behavior changes.
3. Run `go test ./...` and the frontend helper tests. Catalog and template data are
   processed by the same safety invariants as imported templates.
4. Commit the JSON and manifest together and submit them for review. After merge to
...
The v1 catalog intentionally has no hashes or signatures. The HTTPS fixed-origin
...
The v1 SteamCMD subset permits only a positive catalog-owned App ID, anonymous login,
...
Current SteamCMD game directories are `steamcmd/7-days-to-die/` and
`steamcmd/project-zomboid/`. Project Zomboid includes a same-directory, post-start
...
Schema v1 supports the compiled formats `xml-properties` and `ini-key-values`.
