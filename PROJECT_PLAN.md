...
- **v0.5B — Remote Server Management**: IMPLEMENTATION_COMPLETE for the scoped typed Node API, controller API, RBAC/audit, tests, and Nodes UI described below. Binary/runtime-secret forwarding remains excluded.
- **v0.5C — Remote Operational Hardening**: IMPLEMENTATION_COMPLETE for bounded WebSocket console relay with polling fallback, sandboxed text/binary file operations, monitoring, controller UI, RBAC/audit, and tests described below.
- **v0.6 — Cluster / Scheduling (Foundation)**: IMPLEMENTATION_COMPLETE for deterministic placement decisions, live bounded capacity, and explicit native local/remote placement execution. Container decisions remain supported; container creation stays on the existing typed provisioning path. Migration, failover, and controller election remain intentionally deferred.

# v0.5B — Remote Server Management status

Status: IMPLEMENTATION_COMPLETE for the scoped surface. Backend and controller UI are implemented and verified (typed Node API, controller API, RBAC/CSRF/audit/tenant/node isolation, unit/helper tests).

Implemented:

- **Node API** (`internal/api/node_servers.go`, machine-authenticated, additive `remote_server_management` capability): `GET/POST /api/v1/node/servers`, `GET/PATCH/DELETE /api/v1/node/servers/{id}`, `POST .../{start,stop,restart,kill}`. Every call forwards to THIS node's own `internal/servers.Service` - no second lifecycle implementation. Responses are a bounded `nodeServer` projection that never includes the working directory, executable path, arguments, environment variables, stop command, or container config.
- **Controller API** (`internal/api/remoteservers.go`, browser session + CSRF + RBAC): `GET/POST /api/v1/remote-nodes/{id}/servers`, `GET/PATCH/DELETE /api/v1/remote-nodes/{id}/servers/{serverID}`, `POST .../{start,stop,restart,kill}`. Create is global-scope-only (mirrors `Server.Create`'s own restriction, since it carries an arbitrary target-node working directory). Every other action authorizes against the TARGET NODE's own authoritative `tenant_id` (never a cached or client-supplied one) via `authorizeRemoteServer`.
- **Remote client** (`internal/remote/client.go`): typed `ListServers`/`GetServer`/`CreateServer`/`UpdateServer`/`DeleteServer`/`Start|Stop|Restart|KillServer`, plus two new error kinds (`node_resource_not_found`, `node_resource_conflict`) distinct from the generic `node_malformed_response` bucket.
- **RBAC**: new `RemoteServer.View`/`RemoteServer.Manage` permissions, `global`/`tenant` scope only (no local per-remote-server assignment row exists to scope a `server` grant against).
- **Audit**: `remote_server.create/update/delete/start/stop/restart/kill` under a distinct `remote_server` resource type, node id always in metadata; never recorded for list/get reads.
- **Tests**: node-side machine-auth/lifecycle-forwarding tests (`internal/api/node_servers_test.go`), controller-side RBAC/CSRF/tenant-isolation/node-isolation/capability-gating/disabled-node/unreachable-node tests (`internal/api/remoteservers_test.go`), remote-client httptest tests (`internal/remote/client_test.go`).
- See `docs/adr/0010-remote-server-lifecycle-forwarding.md`.

Known limitation:

- The controller UI creates native remote servers through the currently supported structured create contract. Existing Native and Container remote servers can be viewed and controlled; container-specific remote provisioning fields are not forwarded by this milestone.

# v0.5C — Remote Operational Hardening status

Status: IMPLEMENTATION_COMPLETE for the scoped surface. Backend and controller UI are implemented and verified (console/files/monitoring forwarding, separate permissions, tenant/permission isolation tests).

Implemented:

- **Remote console** (`remote_console` capability): a bounded JSON WebSocket relay with polling fallback for older nodes - the node's own console session remains authoritative; `GET/POST /api/v1/node/servers/{id}/console` and the mirrored controller routes remain available. `RemoteConsole.View`/`RemoteConsole.Send` are separate permissions. Console content is never audited - only that input was sent, with a byte count.
- **Remote files** (`remote_files` capability): `GET/PUT/POST/DELETE /api/v1/node/servers/{id}/files[...]` plus bounded multipart upload and binary download, mirrored through the controller. All operations use the SAME `internal/filesystem.Service` sandbox used locally. `RemoteFiles.View/.Edit/.Upload/.Download/.Delete/.Rename` mirror local `Files.*`; binary transfers are capped at 64 MiB.
- **Remote monitoring** (`remote_monitoring` capability): `GET .../monitoring` returns the node's own `MonitoringSnapshot`; never audited, matching the existing "no audit for routine polling" rule.
- **Node isolation, timeouts, no goroutine leaks**: every remote call uses `internal/remote.Client`'s existing bounded timeout/response-size machinery; the polling console design means there is no long-lived connection or background relay goroutine to leak on client disconnect or context cancellation.
- **Tests**: `TestRemoteConsoleSendRequiresSeparatePermission`, `TestRemoteFilesReadOnlyWithoutMutationPermission` (permission separation/read-only enforcement), plus the node/console/files paths exercised in `internal/api/node_servers_test.go`.
- See `docs/adr/0011-remote-operational-hardening.md`.

Known limitations:

- Older enrolled nodes without the new binary/relay implementation continue to use the documented capability-gated polling/text surface.
...
**Explicit gap identified before implementation** (historical, at the time v0.6 was built): v0.5B (Remote Server Create/Edit/Start/Stop) and v0.5C (remote operational hardening) were both NOT STARTED. There was no way, anywhere in this codebase, to create or mutate a server on a Remote Node - only the v0.5A read-only Node API (`GetNodeInfo`/`GetHealth`/`GetCapabilities`) existed. A placement decision could therefore not be executed for a Remote Node target; it could only be proposed. For the LOCAL node, execution was already possible through the existing, unmodified `servers.Service`/provisioning create path, so a `local_only` decision was actionable from the start. **Update**: v0.5B has since implemented a real remote server creation path (see the v0.5B section above and `docs/adr/0010-remote-server-lifecycle-forwarding.md`); a remote selection's `execution` value was renamed from `requires_v0.5b` to `remote_executable` to reflect this - `internal/placement` itself is unchanged and still never executes anything.

**Decision vs. Execution boundary** (see `docs/adr/0009-cluster-scheduling-decision-vs-execution.md`): `POST /api/v1/cluster/placement` remains read-only. Explicit `POST /api/v1/cluster/placement/execute` recomputes placement from current state and then delegates creation to the selected node's normal local `servers.Service.Create` or typed remote Node API. There is no implicit execution, migration, failover, or controller election.

**Capacity model**: the local node and enabled, reachable remote nodes use a live bounded server-list count compared against fixed `DefaultMaxServersPerNode = 50`. If a remote listing fails, capacity remains unknown and the candidate is ranked conservatively. This is intentionally server-count capacity, not resource scheduling.

**API**: `GET /api/v1/cluster/capacity?tenant_id=…`, `POST /api/v1/cluster/placement`, and `POST /api/v1/cluster/placement/execute` (`{tenant_id, runtime_type, server}` → decision plus created server). All use `Cluster.View`/`Cluster.Schedule` with global or tenant scope only; capacity numbers remain node-wide.

**Audit**: one `cluster.placement_decide` event per decision and one `cluster.placement_execute` event per explicit execution outcome; capacity reads and remote health/capacity polling are not audited.

**Not implemented, on purpose**: any generic cron/job scheduler; any change to the unrelated v0.4 local restart scheduler (`internal/scheduler`); server migration between nodes; failover; controller election; resource-based capacity or reservations.

Placement decisions support both `native` and `container` capability matching. Explicit placement execution currently accepts the existing structured native create contract; container creation remains on the established typed provisioning path until a matching remote provisioning contract exists.
