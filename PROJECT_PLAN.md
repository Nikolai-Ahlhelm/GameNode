...
- **v0.5B — Remote Server Management**: NOT STARTED for general remote server CRUD/lifecycle control (start/stop/restart/update/delete an already-existing remote server). v0.6 adds one narrow, typed slice of remote mutation ahead of full v0.5B - see below.
...
- **v0.6 — Cluster / Scheduling (Foundation + Container Placement Execution)**: IMPLEMENTATION_COMPLETE for the scope described below. Placement DECISION, capacity-awareness, a deterministic scheduling algorithm, and EXECUTION of that decision (local and remote, native and container) through the existing typed provisioning path are implemented; migration, failover, controller election, and general remote server management remain deferred.
...
**Remote execution is deliberately narrower than full v0.5B**: there is still no way to start, stop, restart, update, or delete an *already-existing* server on a Remote Node through this product. The only remote-facing mutation v0.6 adds is `POST /api/v1/node/provisioning` (machine-authenticated, on the target node) and its controller-side proxy `POST /api/v1/remote-nodes/{id}/provisioning` - both accept exactly the same bounded, typed fields the existing local provisioning API already accepts (see `internal/provisioning.Request`/`internal/api`'s `provisionInput`), forward straight into the target node's own `provisioning.Service.Start`, and return the same `provisioning.Job` type a local caller already gets. The target node remains the sole authority for template/Egg validation, the image allowlist, resource limits, its tenant/filesystem sandbox, the container installer, job persistence, and final registration through `servers.Service` - the controller only authenticates, checks RBAC/CSRF/tenant scope, forwards the typed request, and returns the bounded job status. There is no raw JSON payload, no generic map of engine flags, and no field for mounts/devices/capabilities/host networking/registry credentials/installer scripts anywhere on this path.
...
**Capacity model** (unchanged from the foundation phase): the local node's usage is a live count of every server `servers.Service` already tracks, compared against a fixed, non-configurable `DefaultMaxServersPerNode = 50` constant. A Remote Node's capacity is always reported unknown - v0.5A's Node API does not expose server counts or resource usage, and this milestone still deliberately does not add a remote listing/capacity call. An unknown-capacity node is still eligible but always ranked behind every node with verified spare capacity.
