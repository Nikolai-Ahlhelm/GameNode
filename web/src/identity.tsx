...
type Role = { id: string; name: string; description: string; permissions?: string[]; server_assignable: boolean; tenant_assignable: boolean };
...
/** TenantAccess is ServerAccess's tenant-scope counterpart, reusing the same
 * rbac.Service assignment infrastructure (GET /tenants/{id}/access, the
 * ordinary POST/DELETE .../roles endpoints) rather than a second engine. */
export function TenantAccess({ tenantID, tenantName, token, capabilities }: { tenantID: string; tenantName?: string; token: string; capabilities?: string[] }) {
  type TenantAssignment = { id: string; role_name: string; subject_type: 'user' | 'group'; subject_id: string; subject_name: string };
  const [items, setItems] = useState<TenantAssignment[]>([]);
  const [users, setUsers] = useState<User[]>([]);
  const [groups, setGroups] = useState<Group[]>([]);
  const [roles, setRoles] = useState<Role[]>([]);
  const [kind, setKind] = useState<'user' | 'group'>('group');
  ...
  const load = async () => {
    try {
      const access = await api<{ assignments: TenantAssignment[] | null }>(`/tenants/${tenantID}/access`);
      setItems(listOrEmpty(access.assignments));
      if (manage) {
        const [userResult, groupResult, roleResult] = await Promise.all([
          api<{ users: User[] | null }>('/users'),
  ...
  useEffect(() => { if (view) void load(); }, [tenantID, view, manage]);
  async function add(event: FormEvent) {
    event.preventDefault();
    try {
      await api(`/${kind === 'user' ? 'users' : 'groups'}/${subject}/roles`, { method: 'POST', headers: csrf(token), body: JSON.stringify({ role_id: role, scope_type: 'tenant', scope_id: tenantID }) });
      setSubject(''); setRole(''); await load();
    } catch (e) { setError(errorMessage(e)); }
  }
  async function remove(item: TenantAssignment) {
    if (!confirm(`Remove ${item.role_name} from ${item.subject_name}?`)) return;
    try { await api(`/${item.subject_type === 'user' ? 'users' : 'groups'}/${item.subject_id}/roles/${item.id}`, { method: 'DELETE', headers: csrf(token) }); await load(); } catch (e) { setError(errorMessage(e)); }
  }
  if (!view) return <p className="hint">Roles.View is required to see this tenant's access assignments.</p>;
  const subjects = kind === 'group' ? groups : users;
  const tenantRoles = roles.filter(item => item.tenant_assignable);
  const excludedRoles = roles.length - tenantRoles.length;
  return <section className="subpanel"><SectionHeader title={`Who has access to ${tenantName || 'this tenant'}?`} description="Tenant-scoped user and group role assignments. A tenant grant applies to every server this tenant owns." />{error && <p className="error notice">{error}</p>}{items.length === 0 ? <EmptyState compact title="No access assignments" description="Assign a tenant-capable role to a user or group below. Tenant membership alone grants no access." /> : <div className="assignment-list">{items.map(item => <div key={item.id}><span>{item.subject_type === 'group' ? 'Group' : 'User'}: {item.subject_name}</span><strong>{item.role_name}</strong>{manage && <button className="danger quiet" onClick={() => void remove(item)}>Remove</button>}</div>)}</div>}{manage && (tenantRoles.length === 0 ? <EmptyState compact title="No tenant-assignable roles available" description="Create a role containing tenant-capable permissions such as Server.View, Server.Create, Console.View, or Files.View, with no global-only permissions." /> : <form className="form-grid assignment-form" onSubmit={add}><label>Subject type<select aria-label="Subject type" value={kind} onChange={e => { setKind(e.target.value as 'user' | 'group'); setSubject(''); }}><option value="group">Group</option><option value="user">User</option></select></label><label>{kind === 'group' ? 'Group' : 'User'}<select aria-label={kind === 'group' ? 'Group' : 'User'} value={subject} onChange={e => setSubject(e.target.value)} required><option value="">Choose {kind}…</option>{subjects.map(item => <option key={item.id} value={item.id}>{'username' in item ? item.username : item.name}</option>)}</select></label><label>Role<select aria-label="Tenant role" value={role} onChange={e => setRole(e.target.value)} required><option value="">Choose role…</option>{tenantRoles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button disabled={!subject || !role}>Assign role</button>{excludedRoles > 0 && <p className="field-help full-field">{excludedRoles} {excludedRoles === 1 ? 'role is' : 'roles are'} hidden because empty roles and roles containing global-only or server-only permissions cannot be assigned to a tenant.</p>}</form>)}</section>;
}
...
function Users({ token, capabilities, isAdmin, passwordPolicy }: { token: string; capabilities?: string[]; isAdmin: boolean; passwordPolicy: { password_minimum_length: number; password_maximum_length: number } }) {
  ...
  return <>
    ...
    {loading ? <SkeletonRows count={5} label="Loading users" /> : items.length === 0 ? <EmptyState title="No users found" description="Create a local user to delegate access." action={rights.manage ? <button onClick={() => setCreating(true)}>Create user</button> : undefined} /> : <div className="data-table identity-table users-table">
...
function AssignmentPanel({ subject, kind, token, capabilities }: { subject: User | Group; kind: 'user' | 'group'; token: string; capabilities?: string[] }) {
  const manage = hasCapability(capabilities, 'Roles.Manage'); const tenantsView = hasCapability(capabilities, 'Tenants.View');
  const [assignments, setAssignments] = useState<Assignment[]>([]); const [roles, setRoles] = useState<Role[]>([]); const [servers, setServers] = useState<Server[]>([]); const [tenants, setTenants] = useState<TenantOption[]>([]);
  const [scope, setScope] = useState<'global' | 'tenant' | 'server'>('global'); const [server, setServer] = useState(''); const [tenant, setTenant] = useState(''); const [role, setRole] = useState(''); const [error, setError] = useState('');
  const route = `/${kind === 'user' ? 'users' : 'groups'}/${subject.id}`;
  const load = async () => {
    try {
      const [a, r] = await Promise.all([api<{ assignments: Assignment[] | null }>(`${route}/roles`), api<{ roles: Role[] | null }>('/roles')]);
      setAssignments(listOrEmpty(a.assignments)); setRoles(listOrEmpty(r.roles));
      try { const response = await api<{ servers: Array<Server | { server: Server }> | null }>('/servers'); setServers(listOrEmpty(response.servers).map(item => 'server' in item ? item.server : item)); } catch { setServers([]); }
      if (tenantsView) { try { setTenants(listOrEmpty((await api<{ tenants: TenantOption[] | null }>('/tenants')).tenants)); } catch { setTenants([]); } }
      setError('');
    } catch (e) { setError(errorMessage(e)); }
  };
  useEffect(() => { void load(); }, [subject.id, kind, tenantsView]);
  async function add(event: FormEvent) { event.preventDefault(); try { await api(`${route}/roles`, { method: 'POST', headers: csrf(token), body: JSON.stringify({ role_id: role, scope_type: scope, ...(scope === 'server' ? { scope_id: server } : scope === 'tenant' ? { scope_id: tenant } : {}) }) }); setRole(''); setServer(''); setTenant(''); await load(); } catch (e) { setError(errorMessage(e)); } }
  ...
  const usableRoles = scope === 'server' ? roles.filter(item => item.server_assignable) : scope === 'tenant' ? roles.filter(item => item.tenant_assignable) : roles;
  const scopeLabel = (assignment: Assignment) => assignment.scope.scope_type === 'global' ? 'Global' : assignment.scope.scope_type === 'tenant' ? (tenants.find(item => item.id === assignment.scope.scope_id)?.name ?? 'Tenant') : (servers.find(item => item.id === assignment.scope.scope_id)?.name ?? 'Server');
  return <section className="detail-card"><SectionHeader title="Access" description="Roles are reusable permission sets; the assignment controls whether they apply globally, to one tenant, or to one server." />{error && <p className="error notice">{error}</p>}{assignments.length === 0 ? <EmptyState compact title="No access assignments" description="No roles are assigned directly to this subject." /> : <div className="assignment-list">{assignments.map(assignment => <div key={assignment.id}><span>{assignment.scope.scope_type !== 'global' && <span className="tenant-badge">{assignment.scope.scope_type === 'tenant' ? 'Tenant' : 'Server'}</span>} {scopeLabel(assignment)}</span><strong>{assignment.role_name}</strong>{manage && <button className="danger quiet" onClick={() => void remove(assignment)}>Remove</button>}</div>)}</div>}{manage && <form className="form-grid assignment-form" onSubmit={add}><label>Scope<select value={scope} onChange={e => { setScope(e.target.value as 'global' | 'tenant' | 'server'); setServer(''); setTenant(''); setRole(''); }}><option value="global">Global — applies everywhere</option>{tenantsView && <option value="tenant">One tenant</option>}<option value="server">One server</option></select></label>{scope === 'tenant' && <label>Tenant<select value={tenant} onChange={e => setTenant(e.target.value)} required><option value="">Choose tenant…</option>{tenants.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}{scope === 'server' && <label>Server<select value={server} onChange={e => setServer(e.target.value)} required><option value="">Choose server…</option>{servers.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}<label>Role<select value={role} onChange={e => setRole(e.target.value)} required disabled={usableRoles.length === 0}><option value="">{usableRoles.length === 0 ? 'No compatible roles' : 'Choose role…'}</option>{usableRoles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button disabled={!role || (scope === 'server' && !server) || (scope === 'tenant' && !tenant)}>Assign role</button>{scope === 'server' && usableRoles.length === 0 && <p className="notice notice--warning full-field">No server-assignable roles exist. Create a role with Server, Console, Files, Ports, or Monitoring permissions and no global-only permissions.</p>}{scope === 'tenant' && usableRoles.length === 0 && <p className="notice notice--warning full-field">No tenant-assignable roles exist. Create a role with tenant-capable permissions such as Server.View or Server.Create and no global-only permissions.</p>}</form>}</section>;
}
...
function RoleEditor({ role, token, manage, onClose }: { role: Role; token: string; manage: boolean; onClose: () => void }) {
  ...
  const tenantSuitability = useMemo(() => roleScopeSuitability(selected, catalog, 'tenant'), [selected, catalog]);
  const serverSuitability = useMemo(() => roleScopeSuitability(selected, catalog, 'server'), [selected, catalog]);
  ...
  return <section><SectionHeader title={role.name} description="Permissions are explicit. Selecting Manage never selects View. Roles stay scope-neutral; the assignment carries the scope." actions={<div className="actions"><button className="quiet" onClick={onClose}>Back</button>{manage && <><button onClick={() => void save()}>Save role</button><button className="danger quiet" onClick={() => void remove()}>Delete</button></>}</div>} /><div className="panel role-editor"><label>Name<input disabled={!manage} value={value.name} onChange={e => setValue({ ...value, name: e.target.value })} /></label><label>Description<input disabled={!manage} value={value.description} onChange={e => setValue({ ...value, description: e.target.value })} /></label><div className="scope-suitability"><div className={`notice ${tenantSuitability.assignable ? 'notice--success' : 'notice--warning'}`}><strong>Tenant: {tenantSuitability.message}</strong>{tenantSuitability.incompatible.length > 0 && <><br /><span>Not tenant-capable: {tenantSuitability.incompatible.join(', ')}</span></>}</div><div className={`notice ${serverSuitability.assignable ? 'notice--success' : 'notice--warning'}`}><strong>Server: {serverSuitability.message}</strong>{serverSuitability.incompatible.length > 0 && <><br /><span>Not server-capable: {serverSuitability.incompatible.join(', ')}</span></>}</div></div><p className="muted">{selected.size} permissions selected · always assignable globally</p>{grouped.map(([category, permissions]) => <section className="permission-group" key={category}><h3>{category}</h3>{permissions.map(permission => <label className="permission" key={permission.key}><input disabled={!manage} type="checkbox" checked={selected.has(permission.key)} onChange={e => setSelected(old => { const next = new Set(old); e.target.checked ? next.add(permission.key) : next.delete(permission.key); return next; })} /><span><strong>{permission.key.split('.')[1]}</strong><small>{permission.description} · {permissionScopeLabel(permission.allowed_scopes)}</small></span></label>)}</section>)}</div>{error && <p className="error notice">{error}</p>}</section>;
}
