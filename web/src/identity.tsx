...
export function IdentityAdmin({ token, capabilities, isAdmin }: { token: string; capabilities?: string[]; isAdmin: boolean }) {
  const visible = {
  ...
  const initial = visible.users ? 'users' : visible.groups ? 'groups' : 'roles';
  ...
  return <section className="identity-page">
    ...
    {tab === 'users' ? <Users token={token} capabilities={capabilities} isAdmin={isAdmin} /> : tab === 'groups' ? <Groups token={token} capabilities={capabilities} /> : <Roles token={token} capabilities={capabilities} />}
  </section>;
}

export function ServerAccess({ serverID, capabilities }: { serverID: string; token: string; capabilities?: string[] }) {
  const [items, setItems] = useState<{ id: string; role_name: string; subject_type: string; subject_name: string }[]>([]);
  const [error, setError] = useState('');
  const view = hasCapability(capabilities, 'Roles.View');
  useEffect(() => { if (view) void api<{ assignments: typeof items }>(`/servers/${serverID}/access`).then(x => setItems(listOrEmpty(x.assignments))).catch(e => setError(errorMessage(e))); }, [serverID, view]);
  if (!view) return null;
  return <section className="subpanel"><SectionHeader title="Who has access to this server?" description="Server-scoped user and group role assignments." />{error && <p className="error notice">{error}</p>}{items.length === 0 ? <EmptyState compact title="No access assignments" description="Assign access from a user or group detail page." /> : <div className="assignment-list">{items.map(item => <div key={item.id}><span>{item.subject_type === 'group' ? 'Group' : 'User'}: {item.subject_name}</span><strong>{item.role_name}</strong></div>)}</div>}</section>;
}

function Users({ token, capabilities, isAdmin }: { token: string; capabilities?: string[]; isAdmin: boolean }) {
  ...
  if (selected) return <UserDetail initial={selected.user} initialSection={selected.section} token={token} capabilities={capabilities} isAdmin={isAdmin} onBack={() => { setSelected(undefined); void load(); }} />;
  return <>
    ...
    {creating && <CreateUser token={token} isAdmin={isAdmin} onCancel={() => setCreating(false)} onCreated={user => { setCreating(false); setSelected({ user }); }} />}
...
function CreateUser({ token, isAdmin, onCancel, onCreated }: { token: string; isAdmin: boolean; onCancel: () => void; onCreated: (user: User) => void }) {
  const [form, setForm] = useState({ username: '', display_name: '', email: '', password: '', confirmation: '', is_admin: false });
  const [error, setError] = useState(''); const [saving, setSaving] = useState(false);
  async function submit(event: FormEvent) {
    event.preventDefault(); const errors = [...validateUserForm(form.username, form.email), ...validatePasswordReset(form.password, form.confirmation)];
...
function UserDetail({ initial, initialSection, token, capabilities, isAdmin, onBack }: { initial: User; initialSection?: 'security'; token: string; capabilities?: string[]; isAdmin: boolean; onBack: () => void }) {
  ...
  return <section className="identity-detail">
    ...
    {manage && <PasswordReset user={user} token={token} autoFocus={initialSection === 'security'} />}
...
function PasswordReset({ user, token, autoFocus }: { user: User; token: string; autoFocus: boolean }) {
  ...
  async function submit(event: FormEvent) { event.preventDefault(); const errors = validatePasswordReset(password, confirmation); if (errors.length) { setError(errors.join(' ')); return; } setSaving(true); setError(''); setNotice(''); try { await api(`/users/${user.id}/password`, { method: 'POST', headers: csrf(token), body: JSON.stringify({ password }) }); setPassword(''); setConfirmation(''); setNotice('Password reset. Existing sessions were invalidated.'); } catch (e) { setError(errorMessage(e)); } finally { setSaving(false); } }
  return <form className="detail-card password-card" onSubmit={submit}><SectionHeader title={`Reset password for ${user.username}`} description="Set a new password without revealing the existing password. All sessions for this user will be invalidated." /><div className="form-grid"><label>New password<input autoFocus={autoFocus} type="password" autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} /></label><label>Confirm password<input type="password" autoComplete="new-password" value={confirmation} onChange={e => setConfirmation(e.target.value)} /></label></div><p className="field-help">Password requirement: 12–256 characters.</p>{error && <p className="error notice">{error}</p>}{notice && <p className="success notice">{notice}</p>}<button disabled={saving || !password}>{saving ? 'Resetting…' : 'Reset password'}</button></form>;
}
...
function AssignmentPanel({ subject, kind, token, capabilities }: { subject: User | Group; kind: 'user' | 'group'; token: string; capabilities?: string[] }) {
  ...
  return <section className="detail-card"><SectionHeader title="Access" description="Global and server-scoped assignments use the shared RBAC assignment APIs." />{error && <p className="error notice">{error}</p>}{assignments.length === 0 ? <EmptyState compact title="No access assignments" description="No roles are assigned directly to this subject." /> : <div className="assignment-list">{assignments.map(assignment => <div key={assignment.id}><span>{scopeLabel(assignment)}</span><strong>{assignment.role_name}</strong>{manage && <button className="danger quiet" onClick={() => void remove(assignment)}>Remove</button>}</div>)}</div>}{manage && <form className="form-grid assignment-form" onSubmit={add}><label>Scope<select value={scope} onChange={e => { setScope(e.target.value as 'global' | 'server'); setServer(''); }}><option value="global">Global</option><option value="server">Server</option></select></label>{scope === 'server' && <label>Server<select value={server} onChange={e => setServer(e.target.value)} required><option value="">Choose server…</option>{servers.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label>}<label>Role<select value={role} onChange={e => setRole(e.target.value)} required><option value="">Choose role…</option>{usableRoles.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></label><button>Assign role</button></form>}</section>;
}
...
function Roles({ token, capabilities }: { token: string; capabilities?: string[] }) {
  ...
  return <><SectionHeader title="Roles" description="Reusable permission sets. Manage permissions never imply View." actions={manage ? <button onClick={() => setCreating(true)}>Create role</button> : undefined} />{error && <p className="error notice">{error}</p>}{creating && <CreateRole token={token} onDone={() => { setCreating(false); void load(); }} />}{roles.length === 0 ? <EmptyState title="No custom roles yet" description="Create a role to define reusable permission sets." /> : <div className="data-table role-table"><div className="table-head"><span>Role</span><span>Description</span><span>Actions</span></div>{roles.map(item => <div className="table-row" key={item.id}><strong>{item.name}</strong><span>{item.description || '—'}</span><span><button className="quiet" onClick={() => setSelected(item)}>Open</button></span></div>)}</div>}</>;
}

function CreateRole({ token, onDone }: { token: string; onDone: () => void }) {
  const [name, setName] = useState(''); const [description, setDescription] = useState(''); const [error, setError] = useState('');
  async function submit(event: FormEvent) { event.preventDefault(); try { await api('/roles', { method: 'POST', headers: csrf(token), body: JSON.stringify({ name, description }) }); onDone(); } catch (e) { setError(errorMessage(e)); } }
  return <form className="panel form-panel identity-form" onSubmit={submit}><label>Name<input value={name} onChange={e => setName(e.target.value)} required /></label><label>Description<input value={description} onChange={e => setDescription(e.target.value)} /></label>{error && <p className="error notice">{error}</p>}<div className="actions"><button>Create role</button><button type="button" className="quiet" onClick={onDone}>Cancel</button></div></form>;
}
