...
export type SettingsForm = { sampleInterval: string; historyLimit: string; logLevel: 'debug' | 'info' | 'warn' | 'error'; passwordMinimumLength: string; passwordMaximumLength: string; brandingName: string; brandingSubtitle: string };
...
export const settingsForm = (value: SettingsResponse): SettingsForm => ({ sampleInterval: String(value.monitoring.sample_interval_seconds), historyLimit: String(value.monitoring.history_limit), logLevel: value.logging?.level ?? 'info', passwordMinimumLength: String(value.security.password_minimum_length), passwordMaximumLength: String(value.security.password_maximum_length), brandingName: value.branding.name, brandingSubtitle: value.branding.subtitle });
export const settingsPatch = (current: SettingsResponse, form: SettingsForm) => {
  ...
  return Object.keys(monitoring).length || logging || Object.keys(security).length || Object.keys(branding).length ? { ...(Object.keys(monitoring).length ? { monitoring } : {}), ...(logging ? { logging } : {}), ...(Object.keys(security).length ? { security } : {}), ...(Object.keys(branding).length ? { branding } : {}) } : undefined;
};
